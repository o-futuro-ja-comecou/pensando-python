...
together creates an effect or emotional response.
...
The types we have seen so far are integers (type `int`), floatingpoint numbers (type `float`), and strings (type `str`).
